...
// Platform themes
use oat\taoCuriosity\model\theme\TaocuriosityDefaultTheme;
...
foreach($itemThemes as $themeName => $isDefault){
    ...
    ThemeRegistry::getRegistry()->registerTheme(
        $themeId,
        $themeName,
        implode(DIRECTORY_SEPARATOR, array('taoCuriosity', 'views', 'css', 'themes', 'items', $pathFragment, 'theme.css')), array('items')
    );
}
...
$themeService->setTheme(new TaocuriosityDefaultTheme());
